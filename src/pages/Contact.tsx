...
import { MapPin, Phone, Mail, Clock, Instagram, Youtube, Loader2, CheckCircle2 } from 'lucide-react';
    ...
    <div className="pt-20 min-h-screen bg-brand-black">
      <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[calc(100vh-80px)]">
        <div className="p-8 md:p-16 lg:p-24 flex flex-col justify-center">
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center justify-center p-12 bg-brand-charcoal border border-white/10 rounded-lg text-center h-full">
              <CheckCircle2 className="h-16 w-16 text-brand-accent mb-4" />
              <h2 className="text-3xl font-heading text-white mb-4">Message Sent!</h2>
              <p className="text-gray-400 mb-8 max-w-md">
                Thanks for reaching out. We've received your message and will get back to you via email shortly.
              </p>
              <Button onClick={() => setStatus('idle')} className="bg-brand-accent hover:bg-white hover:text-brand-black text-brand-black font-bold">
                ...
                try {
                  const res = await fetch('https://formsubmit.co/ajax/info@nyalicrossfitgym.com', {
                    ...
                    body: JSON.stringify({
                      ...
                      email: data.email,
                      _subject: `Website Contact Form: ${data.firstName} ${data.lastName}`,
                      _template: 'table',
            ...
            >
              ...
              <Button type="submit" disabled={status === 'submitting'} size="lg" className="w-full bg-brand-accent hover:bg-white hover:text-brand-black text-brand-black border-none font-bold flex items-center justify-center gap-2">
                {status === 'submitting' ? (
